class SessionController < ApplicationController
  def auth
    ...
    if !user || !user.valid_token?
      ...
      redirect_to root_path, notice: 'Your login token has expired. Try requesting a new login link'
    else
      sign_in_user(user)
  ...
  def create
    ...
    if !user
      redirect_to new_session_path, notice: "Uh oh! We couldn't find the username or the email. Please try again"
    else
